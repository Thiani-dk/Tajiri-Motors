...
export default function AdminPage() {
  ...
  const handleLogin = (e: any) => {
    e.preventDefault()
    if (pinInput === SECRET_PIN) setIsAuthenticated(true)
  ...
  const handleDelete = async (id: number) => {
    ...
    if (error) alert("Error deleting car")
    else fetchInventory() 
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700 h-fit">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><label className={labelClass}>Price (KSH)</label><input required type="number" name="price" placeholder="0" value={formData.price} onChange={handleChange} className={`${inputClass} font-bold text-green-700 dark:text-green-400`} /></div>
              <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
              {inventory.length === 0 ? <p className="text-gray-400 text-center py-10">No cars in stock.</p> : inventory.map((car) => (
                <div key={car.id} className="flex justify-between items-center p-3 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 dark:border-gray-600 transition">
                  <div className="flex items-center gap-3">
                    ...
                    <div><p className="font-bold text-sm">{car.year} {car.make} {car.model}</p><p className="text-xs text-green-600 font-bold">{formatPrice(car.price)}</p></div>
                  </div>
                  <button onClick={() => handleDelete(car.id)} className="text-red-400 hover:text-red-600 p-2 hover:bg-red-50 rounded-full transition"><Trash2 size={18} /></button>
                </div>
              ))}
            </div>
          </div>
